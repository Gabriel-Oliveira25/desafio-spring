...
public class VehicleServices {
	...
	public GenericResponseDTO registerVehicle(VehicleRequestDTO request) {
		Optional.ofNullable(this.repository.findByPlateNumber(request.getPlateNumber()))
	...
	public GenericResponseDTO deleteVehicle(String id) {
		Optional.ofNullable(this.repository.findById(id).orElse(null)).ifPresentOrElse(vehicle -> {
	...
	public Client getClientsByVehicleId(String id) {
		return this.repository.findById(id).orElseThrow(() -> new GeneralException("Client not found in our database", HttpStatus.NOT_FOUND)).getClient();
	}
}
